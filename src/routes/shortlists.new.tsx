...
export const Route = createFileRoute("/shortlists/new")({
  ...
  validateSearch: (s: Record<string, unknown>): { client?: string; job?: string } => ({
    client: s.client as string | undefined,
    job: s.job as string | undefined,
  }),
...
function NewShortlist() {
  ...
  const [brand, setBrand] = useState<"moove" | "portus" | undefined>();
  ...
  useEffect(() => {
    if (clientId)
      numFn({ data: { client_id: clientId } }).then((r: any) =>
        setMeta((m) => ({
          ...m,
          number: r.number,
          title: m.title || `Shortlist ${String(r.number).padStart(2, "0")}`,
        })),
      );
  }, [clientId]);

  // Pré-seleciona a marca cadastrada no cliente, mas só enquanto a recrutadora
  // não tiver escolhido manualmente — depois disso a escolha dela prevalece.
  useEffect(() => {
    if (brandTouched) return;
    const c = clients?.find((x: any) => x.id === clientId);
    if (c?.brand === "moove" || c?.brand === "portus") setBrand(c.brand);
  }, [clientId, clients, brandTouched]);
  ...
  const filteredJobs = clientId
    ? (jobs ?? []).filter((j: any) => j.client_id === clientId)
    : (jobs ?? []);

  const persistDraft = async () => {
    if (!clientId || !jobId) {
      toast.error("Cliente e vaga são obrigatórios");
      return null;
    }
    if (!brand) {
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="card-elevated p-6 min-h-[380px]">
                ...
                {clients?.map((c: any) => (
                  ...
                  >
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">
                      {c.contact_name || c.segment || "—"}
                    </div>
                  </button>
                ))}
                {clients?.length === 0 && (
                  <div className="text-sm text-muted-foreground">
                    Nenhum cliente. Cadastre um primeiro.
                  </div>
                )}
              </div>
            </div>
          ...
          {step === 1 && (
            <div>
              <div className="mb-3 flex items-center justify-between">
                ...
                <a
                  onClick={() => navigate({ to: "/jobs/new", search: { client: clientId } })}
                  className="text-xs text-primary hover:underline cursor-pointer"
                >
                  + Nova vaga
                </a>
              ...
              <div className="grid gap-3">
                ...
                {filteredJobs.length === 0 && (
                  <div className="text-sm text-muted-foreground">
                    Nenhuma vaga para este cliente.
                  </div>
                )}
              </div>
            </div>
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div>
                <Label>Esta shortlist será de qual marca?</Label>
                <div className="mt-2 grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={cn(
                      ...
                      brand === "moove"
                        ? "border-[#7412DE] bg-[#7412DE]/5 ring-1 ring-[#7412DE]"
                        : "border-border bg-card hover:border-[#7412DE]/40",
                    )}
                  >
                    <div
                      className="font-semibold"
                      style={{ color: brand === "moove" ? "#7412DE" : undefined }}
                    >
                      Moove Talent
                    </div>
                    <div className="mt-0.5 text-xs text-muted-foreground">
                      Identidade visual Moove Talent
                    </div>
                  </button>
                  <button
                    type="button"
                    ...
                    className={cn(
                      "rounded-xl border p-4 text-left transition",
                      brand === "portus"
                        ? "border-[#009B76] bg-[#009B76]/5 ring-1 ring-[#009B76]"
                        : "border-border bg-card hover:border-[#009B76]/40",
                    )}
                  >
                    <div
                      className="font-semibold"
                      style={{ color: brand === "portus" ? "#009B76" : undefined }}
                    >
                      Portus
                    </div>
                    <div className="mt-0.5 text-xs text-muted-foreground">
                      Identidade visual Portus
                    </div>
                  </button>
                </div>
                <div className="mt-1.5 text-xs text-muted-foreground">
                  Define a identidade visual da shortlist no portal do cliente. Fica travada nesta
                  shortlist a partir da publicação.
                </div>
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <Label>Título da shortlist</Label>
                  <Input
                    value={meta.title}
                ...
                <div>
                  ...
                  <Input
                    type="date"
                    value={meta.send_date}
                    onChange={(e) => setMeta({ ...meta, send_date: e.target.value })}
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setMeta({ ...meta, responsible: e.target.value })}
                  />
                </div>
              </div>
              <div>
                ...
                <Textarea
                  rows={4}
                  value={meta.message}
                  onChange={(e) => setMeta({ ...meta, message: e.target.value })}
                />
              </div>
            </div>
          )}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      Envie currículo, foto, DISC e parecer — a IA gera o perfil.
                    </div>
                  </div>
                </div>
                <Button
                  ...
                  onClick={async () => {
                    const id = await persistDraft();
                    if (id)
                      navigate({ to: "/candidates/new", search: { job: jobId, shortlist: id } });
                  }}
                >
                  Novo candidato
                </Button>
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={saveDraftAndBack}>
              Salvar rascunho
            </Button>
            {step < 3 ? (
              <Button
                disabled={
                  (step === 0 && !clientId) || (step === 1 && !jobId) || (step === 2 && !brand)
                }
              ...
              <Button disabled={selected.length === 0 || !brand} onClick={goPublish}>
                Revisar e publicar
              </Button>
            )}
          </div>
